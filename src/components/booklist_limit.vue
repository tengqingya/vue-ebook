...
<template>
  <div>
    ...
    <div>
      <bookitem :book="item" :key="index" v-for="(item,index) in books" @click.native="clickBook(item)" :type="2">
...
<script>
  import {getlimitgroupbooks} from '../axios/axios'
  ...
  export default {
    ...
    methods: {
      ...
      getAjax(){
        getlimitgroupbooks({id:this.subject_id}).then(e=>{
